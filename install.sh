...
if ! sudo systemctl start mariadb 2>&1; then
  ...
  sudo tail -50 /var/lib/mysql/*.err 2>/dev/null || true
  exit 1
fi
...
for i in {1..60}; do
  ...
  if [ $i -eq 60 ]; then
    echo
    echo -e "${RED}MariaDB did not start within 60 seconds.${NC}"
...
mysql -u root -p"$ROOT_MYSQL_PASS" --socket="$MYSQL_SOCKET" <<SQL
